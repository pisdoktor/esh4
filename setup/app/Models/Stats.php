...
class Stats extends BaseModel {
    ...
    {
        return TenantContext::scopeCacheKey() . '_' . preg_replace('/[^a-zA-Z0-9_-]/', '', $key);
    }
    ...
    private function filterPartsWithKurum(array $parts, string $hAlias = 'h', ?string $eAlias = null): array
    {
        ...
        if (count($ids) === 1) {
            $parts['where'] .= ' AND ' . $hAlias . '.kurum_id = ' . (int) $ids[0];
        } else {
            $parts['where'] .= ' AND ' . $hAlias . '.kurum_id IN (' . implode(',', array_map('intval', $ids)) . ')';
        }
        if ($eAlias !== null && ($parts['from'] ?? '') === 'erapor') {
            if (count($ids) === 1) {
                $parts['where'] .= ' AND ' . $eAlias . '.kurum_id = ' . (int) $ids[0];
            } else {
                $parts['where'] .= ' AND ' . $eAlias . '.kurum_id IN (' . implode(',', array_map('intval', $ids)) . ')';
            }
        }
    ...
    private function dataHealthPatientFilter(string $metric): ?array {
        ...
        if ($ids !== null && count($ids) === 1) {
            $izKurumSub = ' WHERE kurum_id = ' . (int) $ids[0];
        } elseif ($ids !== null && count($ids) > 1) {
            $izKurumSub = ' WHERE kurum_id IN (' . implode(',', array_map('intval', $ids)) . ')';
        } else {
            $izKurumSub = '';
        }
        $pasif = "h.pasif = '0'";
    ...
    /**
     * Barthel — son değerlendirme skor dağılımı (aktif hastalar).
    ...
    public function getBarthelDistribution(): object {
        $riskCase = 'CASE
                WHEN b.toplam_skor <= 20 THEN \'g_0_20\'
                WHEN b.toplam_skor <= 61 THEN \'g_21_61\'
                WHEN b.toplam_skor <= 90 THEN \'g_62_90\'
                WHEN b.toplam_skor < 100 THEN \'g_91_99\'
                ELSE \'g_100\'
            END';
        $labels = [
            'g_0_20' => 'Tam bağımlı (0–20)',
            'g_21_61' => 'Ağır (21–61)',
            'g_62_90' => 'Orta (62–90)',
            'g_91_99' => 'Hafif (91–99)',
            'g_100' => 'Tam bağımsız (100)',
        ];
        $report = $this->fetchClinicalScaleDistribution(
            '#__hasta_barthel',
            "h.pasif = '0'",
            $riskCase,
            $labels
        );
        $gruplar = (array) ($report->gruplar ?? []);

        return (object) [
            'toplam_hasta' => (int) ($report->degerlendirilen_hasta ?? 0),
            'ortalama_skor' => (float) ($report->ortalama_skor ?? 0.0),
            'g_0_20' => (int) ($gruplar[$labels['g_0_20']] ?? 0),
            'g_21_61' => (int) ($gruplar[$labels['g_21_61']] ?? 0),
            'g_62_90' => (int) ($gruplar[$labels['g_62_90']] ?? 0),
            'g_91_99' => (int) ($gruplar[$labels['g_91_99']] ?? 0),
            'g_100' => (int) ($gruplar[$labels['g_100']] ?? 0),
        ];
    }
