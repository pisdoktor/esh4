...
/**
 * Adres Tablosu Modeli — bölge/ilçe/mahalle/sokak/kapı platform geneli (tüm kurumlar ortak).
 * Kurum bazlı mahalle planı (bölge/gün): #__mahalle_plan + MahallePlan modeli.
 */
class Address extends BaseModel {

    /** Migrasyon ve Denizli senkron varsayılan kök bölge kaydı. */
    public const DEFAULT_BOLGE_ID = '00000000-0000-4000-a000-adrestanim001';
    ...
    /**
     * Kök bölge kayıtları (ust_id = 0).
     *
     * @return list<object>
     */
    public function getBolgeler(): array {
        $rows = $this->db->fetchObjectListPrepared(
            'SELECT a.id, a.adi FROM ' . $this->_tbl . ' AS a WHERE a.tip = ? AND a.ust_id = ? ORDER BY a.adi ASC',
            ['bolge', '0']
        );
    ...
    /**
     * Varsayılan veya ilk bölge kaydının id'si (Denizli ilçe import / migrasyon).
     */
    public function resolveDefaultBolgeId(): string {
        $row = $this->db->fetchObjectPrepared(
            'SELECT id FROM ' . $this->_tbl . ' WHERE id = ? AND tip = ? LIMIT 1',
            [self::DEFAULT_BOLGE_ID, 'bolge']
        );
        if ($row && !empty($row->id)) {
            return (string) $row->id;
        }
        $row = $this->db->fetchObjectPrepared(
            'SELECT id FROM ' . $this->_tbl . ' WHERE tip = ? ORDER BY adi ASC, id ASC LIMIT 1',
            ['bolge']
        );
        if ($row && !empty($row->id)) {
            return (string) $row->id;
    ...
    */
    ...
    public function adminListByTipUst($tip, $ustId) {
        ...
        if ($tip === 'bolge') {
            ...
            $params = ['bolge', '0'];
            $fedFilter = self::effectiveFederationBolgeFilterForAdminList();
            if ($fedFilter !== null) {
                $sql .= ' AND a.federation_bolge_id = ?';
                $params[] = $fedFilter;
            }
    ...
    /**
     * Üst menü bölge filtresi (PS / bölge kilitli süper yönetici) — Adrestanim bolge listesi.
     */
    private static function effectiveFederationBolgeFilterForAdminList(): ?int
    {
        if (!FederationAdresBolgeSync::columnReady()) {
            return null;
        }
        $fedId = TenantContext::effectiveBolgeFilterId();
        if ($fedId === null || $fedId <= 0) {
            return null;
        }
    ...
    {
        return MapRoutingGeocodeHelper::firstPosition($addressQuery);
    }
