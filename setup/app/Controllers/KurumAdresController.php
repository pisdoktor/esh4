...
{
    private static array $validTips = ['bolge', 'ilce', 'mahalle', 'sokak'];
    ...
    public function index(): void
    {
        ...
        $kurumlar = TenantContext::kurumListForScope(true);
        if ($kurumId <= 0 && $kurumlar !== []) {
            $kurumId = (int) ($kurumlar[0]->id ?? 0);
        } elseif ($kurumId > 0 && !TenantContext::isKurumInScope($kurumId)) {
            $kurumId = $kurumlar !== [] ? (int) ($kurumlar[0]->id ?? 0) : 0;
        }
    ...
    {
        $tip = isset($_GET['tip']) ? trim((string) $_GET['tip']) : 'bolge';
    ...
    public function ajaxAssign(): void
    {
        ...
        if (!TenantContext::isKurumInScope($kurumId)) {
            $this->jsonOut(['ok' => false, 'error' => 'Bu kuruma erişim yetkiniz bulunmamaktadır.']);
        }
