...
class KurumController
{
    ...
    public function indexRows(): void
    {
        ...
        $sort = self::kurumListSort();
        $items = \App\Helpers\TenantContext::kurumListForScope(false, $sort['orderFragment']);
    ...
    public function edit(): void
    {
        ...
        TenantContext::assertRecordKurum($id);
        include ThemeViewHelper::resolvePartial('header');
    ...
    public function store(): void
    {
        ...
        if (\App\Helpers\FederationHelper::columnsReady()) {
            if (TenantContext::sessionIsBolgeLockedSuperAdmin()) {
                $assigned = TenantContext::sessionAssignedBolgeId();
                $model->set('bolge_id', $assigned !== null && $assigned > 0 ? $assigned : null);
            } else {
                $bolgeRaw = $_POST['bolge_id'] ?? null;
                $bolgeId = is_numeric($bolgeRaw) ? (int) $bolgeRaw : 0;
                $model->set('bolge_id', $bolgeId > 0 ? $bolgeId : null);
            }
            $model->set('federation_ref', \App\Helpers\FederationHelper::normalizeRef((string) ($_POST['federation_ref'] ?? '')));
        }
