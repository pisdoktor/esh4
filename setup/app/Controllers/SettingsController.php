...

{
    ...
    private const PLATFORM_OWNER_TABS = [
...

    {
        ...
        if (AuthHelper::sessionIsSuperAdmin()) {
            ...
            return array_merge(self::KURUM_TABS, self::SUPERADMIN_TABS);
        ...
        if ($activeTab === 'islem_ids') {
            ...
            if ($r1 !== true) {
                ...
                if ($r2 !== true) {
                    ...
                    $rFieldVisit = OperationalSettings::saveSection('field_visit', $posted);

                    if ($rFieldVisit !== true) {

                        $errors[] = is_string($rFieldVisit) ? $rFieldVisit : 'Saha ziyareti ayarları kaydedilemedi.';

                    } else {

                    $rUhdsTele = OperationalSettings::saveSection('uhds_telehealth', $posted);

                    if ($rUhdsTele !== true) {

                        $errors[] = is_string($rUhdsTele) ? $rUhdsTele : 'UHDS görüntülü görüşme ayarları kaydedilemedi.';
                    ...
                    if ($r3 === true) {
                        ...
                        $_SESSION['success'] = 'Planlama, süre, saha ziyareti, UHDS görüşme ve vardiya ayarları kaydedildi. Yeni isteklerde geçerlidir.';
            ...
            $rEsysBridge = OperationalSettings::saveSection('esys_bridge', OperationalSettings::postedForSection('esys_bridge', $posted));

            if ($rEsysBridge !== true) {

                $errors[] = is_string($rEsysBridge) ? $rEsysBridge : 'ESYS köprüsü ayarları kaydedilemedi.';

            }

            $rUsbsBridge = OperationalSettings::saveSection('usbs_bridge', OperationalSettings::postedForSection('usbs_bridge', $posted));

            if ($rUsbsBridge !== true) {

                $errors[] = is_string($rUsbsBridge) ? $rUsbsBridge : 'USBS köprüsü ayarları kaydedilemedi.';

            }

            $rFederation = OperationalSettings::saveSection('federation', OperationalSettings::postedForSection('federation', $posted));

            if ($rFederation !== true) {

                $errors[] = is_string($rFederation) ? $rFederation : 'Federasyon ayarları kaydedilemedi.';
            ...
            if ($rClinical !== true) {
                ...
                $errors[] = is_string($rClinical) ? $rClinical : 'Klinik karar desteği ayarları kaydedilemedi.';

            }

            $rAuditLog = OperationalSettings::saveSection('audit_log', OperationalSettings::postedForSection('audit_log', $posted));

            if ($rAuditLog !== true) {

                $errors[] = is_string($rAuditLog) ? $rAuditLog : 'Denetim günlüğü ayarları kaydedilemedi.';

            }

            if ($rMaintenance === true && $rPublic === true && $rPortal === true && $rEsysBridge === true && $rUsbsBridge === true && $rFederation === true && $rModern === true && $rClinical === true && $rAuditLog === true) {
