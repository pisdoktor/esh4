...
class StatsController {
    ...
    public function reportPdfData(): void {
        ...
        AuditLogHelper::statsExport(['report' => $report]);
    ...
    public function dataHealthContent() {
        ...
        $clinicalOverdueHighRiskCount = ClinicalDecisionSupportHelper::enabled()
            ? (new ClinicalDecisionSupportService())->countOverdueHighRisk()
            : 0;
    ...
    public function clinicalDecisionSupport() {
        if (!ClinicalDecisionSupportHelper::enabled()) {
            $_SESSION['error'] = 'Klinik karar desteği kapalı. Ayarlar → Güvenlik bölümünden açabilirsiniz.';
            header('Location: ' . esh_url('Stats', 'index'));
            exit;
        }
        $service = new ClinicalDecisionSupportService();
        $totalCount = $service->countOverdueHighRisk();
        $rows = $service->listOverdueHighRisk(100, 0);
        $pageTitle = 'Klinik karar desteği';
        $this->render('clinical_decision_support', $pageTitle, [
            'overdueDays' => ClinicalDecisionSupportHelper::overdueDays(),
