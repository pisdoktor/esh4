...
final class IslemIdSettings
{
    ...
    public static function saveValues(array $posted)
    {
        ...
        if (($target['target'] ?? '') === SettingsWriteScope::TARGET_BOLGE) {
            ...
            $existing = BolgeCorporateSettings::getIslemIdsMap($bolgeId);
            $merged = array_merge($existing, $toSave);
            foreach (self::$catalog ?? [] as $item) {
                $key = (string) ($item['key'] ?? '');
                if ($key !== '' && !array_key_exists($key, $merged)) {
            ...
            $merged = self::filterIslemIdsToCatalog($merged);
            $written = BolgeCorporateSettings::saveIslemIds($bolgeId, $merged);
            if ($written !== true) {
                return $written;
            }
            self::$resolved = self::buildResolvedMap();
    ...
    private static function resolveRaw(string $key)
    {
        ...
        if (class_exists(BolgeCorporateSettings::class)) {
            $bolgeMap = BolgeCorporateSettings::getIslemIdsMap();
            if (array_key_exists($key, $bolgeMap)) {
                return $bolgeMap[$key];
            }
        }
