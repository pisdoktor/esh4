...
class DenizliAdresSync {
    ...
    public static function getTableCounts(): array {
        ...
        $out = ['bolge' => 0, 'ilce' => 0, 'mahalle' => 0, 'sokak' => 0, 'kapino' => 0];
        foreach ($rows as $row) {
            $tip = (string) ($row['tip'] ?? '');
            if (isset($out[$tip])) {
                $out[$tip] = (int) ($row['c'] ?? 0);
    ...
    public static function createJob(bool $truncate): array {
        ...
        $job = [
            ...
            'sokak_cursor' => null,
            'http_fail_count' => 0,
            'totals' => ['bolge' => 0, 'ilce' => 0, 'mahalle' => 0, 'sokak' => 0, 'kapino' => 0],
    ...
    private static function ensureDefaultBolge(Database $db): string {
        $addr = new Address();
        $bolgeId = $addr->resolveDefaultBolgeId();
        $exists = $db->fetchObjectPrepared(
            'SELECT id FROM #__adrestablosu WHERE id = ? LIMIT 1',
            [$bolgeId]
        );
        if (!$exists) {
            $db->executePrepared(
                'INSERT INTO #__adrestablosu (id, adi, ust_id, tip) VALUES (?, ?, ?, ?)',
                [$bolgeId, 'Varsayılan Bölge', '0', 'bolge']
            );
        }
    ...
    private static function stepIlce(Database $db, array &$job): void {
        ...
        $bolgeId = self::ensureDefaultBolge($db);
        $job['totals']['bolge'] = 1;
        $ilceler = self::fetchIlceListFromApi();
        $stats = self::upsertRows($db, $ilceler, $bolgeId, 'ilce');
