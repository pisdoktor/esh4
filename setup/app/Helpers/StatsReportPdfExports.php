...
final class StatsReportPdfExports {
    ...
    public static function tryExport(string $action, array $query): ?array {
        ...
        return match ($action) {
            ...
            'clinicalDecisionSupport' => self::exportClinicalDecisionSupport(),
    ...
    private static function exportClinicalDecisionSupport(): ?array
    {
        if (!ClinicalDecisionSupportHelper::enabled()) {
            return null;
        }
        $svc = new ClinicalDecisionSupportService();
        $rows = [];
        foreach ($svc->listOverdueHighRisk(300, 0) as $r) {
            $rows[] = [
                trim((string) ($r->isim ?? '') . ' ' . (string) ($r->soyisim ?? '')),
                ValidationHelper::formatTc((string) ($r->tckimlik ?? '')),
                (string) ($r->risk_summary ?? ''),
                (string) ($r->son_yapilan_tarih ?? ''),
            ];
        }

        return StatsReportPdfFormatHelper::payload(
            'Klinik karar desteği',
            ['Hasta', 'TC', 'Risk özeti', 'Son yapılan izlem'],
            $rows,
            'Yüksek riskli izlenmeyen hastalar — ' . ClinicalDecisionSupportHelper::overdueDays() . ' gün',
            StatsReportPdfFormatHelper::filename('clinicalDecisionSupport')
        );
    }
