<div class="card border-0 shadow-sm rounded-3 mb-3 overflow-hidden">
        <div class="card-header bg-white py-3 px-3 px-md-4 border-bottom d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <button
                id="stats-randevu-takvim-filter-toggle"
                class="btn btn-outline-secondary btn-sm rounded-pill px-3<?= $filterExpanded ? '' : ' collapsed' ?>"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#stats-randevu-takvim-filter-collapse"
                aria-expanded="<?= $filterExpanded ? 'true' : 'false' ?>"
                aria-controls="stats-randevu-takvim-filter-collapse"
            >
        ...
        <div id="stats-randevu-takvim-filter-collapse" class="collapse<?= $filterExpanded ? ' show' : '' ?>">
        <div class="card-body p-3 p-md-4 bg-body-tertiary bg-opacity-25">
            <form class="row g-3 g-xl-4 align-items-end" method="get" action="<?= htmlspecialchars(esh_form_action('Stats', 'randevuTakvim'), ENT_QUOTES, 'UTF-8') ?>">
                <?= esh_form_route_hiddens('Stats', 'randevuTakvim') ?>
                ...
                <div class="col-12 col-md-6 d-flex gap-2">
                    ...
                    <a href="<?= htmlspecialchars(esh_url('Stats', 'randevuTakvim'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-secondary btn-sm rounded-pill px-3">Sıfırla</a>
                </div>
            </form>
        </div>
        </div>
    ...
    <div class="row g-4">
        <div class="col-xl-6">
            <?php
            $sectionTitle = 'Branş randevu takvimi';
            $sectionIcon = 'fa-user-doctor';
            $calendarUrl = esh_url('Randevu', 'index');
            $report = $kons;
            $chartId = 'konsRandevuChart';
            $durumPositiveLabel = 'Geldi';
            $durumNegativeLabel = 'Gelmedi';
            $eshStatsPdfBlock = 'kons';
            require $partial;
            ?>
        </div>
        <div class="col-xl-6">
            ...
            $calendarUrl = esh_url('Uhds', 'index');
            $report = $uhds;
            $chartId = 'uhdsRandevuChart';
            $durumPositiveLabel = 'Yapıldı';
            $durumNegativeLabel = 'Yapılmadı';
            $eshStatsPdfBlock = 'uhds';
            require $partial;
            ?>
        </div>
    </div>
