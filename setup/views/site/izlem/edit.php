...
$visitCheckinLat = isset($visit->checkin_lat) && is_numeric($visit->checkin_lat) ? (float) $visit->checkin_lat : null;
$visitCheckinLon = isset($visit->checkin_lon) && is_numeric($visit->checkin_lon) ? (float) $visit->checkin_lon : null;
$hasExistingCheckin = $visitCheckinLat !== null && $visitCheckinLon !== null
    && $visitCheckinLat >= -90 && $visitCheckinLat <= 90
    && $visitCheckinLon >= -180 && $visitCheckinLon <= 180;
?>
    ...
    <div class="card border-0 shadow rounded-3 overflow-hidden">
        ...
        <div class="card-body p-0">
            ...
            <form id="form-izlem-edit-<?= $vid ?>" class="p-4" action="<?= htmlspecialchars(esh_url('Visit', 'update'), ENT_QUOTES, 'UTF-8') ?>" method="post">
                ...
                <input type="hidden" name="checkin_lat" value="<?= $hasExistingCheckin ? htmlspecialchars((string) $visitCheckinLat, ENT_QUOTES, 'UTF-8') : '' ?>">
                <input type="hidden" name="checkin_lon" value="<?= $hasExistingCheckin ? htmlspecialchars((string) $visitCheckinLon, ENT_QUOTES, 'UTF-8') : '' ?>">
                <input type="hidden" name="checkin_at" value="<?= htmlspecialchars((string) ($visit->checkin_at ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                <input type="hidden" name="checkin_accuracy" value="<?= isset($visit->checkin_accuracy) && is_numeric($visit->checkin_accuracy) ? htmlspecialchars((string) $visit->checkin_accuracy, ENT_QUOTES, 'UTF-8') : '' ?>">
                ...
                <?php include __DIR__ . '/partials/esys_ref_panel.php'; ?>

                <?php include __DIR__ . '/partials/usbs_ref_panel.php'; ?>
                ...
                <div class="izlem-mobile-actions d-lg-none d-flex flex-wrap gap-2 justify-content-between align-items-stretch">
                    <a href="<?= htmlspecialchars(esh_url('Visit', 'history', ['tc' => urlencode($patient->tckimlik ?? '')]), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-secondary flex-grow-1 rounded-pill">Vazgeç</a>
                    <button type="submit" class="btn btn-primary flex-grow-1 rounded-pill shadow-sm fw-semibold">
                        <i class="fa-solid fa-floppy-disk me-2"></i>Güncelle
                    </button>
                </div>
            </form>
