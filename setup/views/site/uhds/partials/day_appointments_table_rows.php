    ...
    <tr>
        ...
        <td class="text-center">
            <?php if ($uhdsTelehealthEnabled): ?>
                <a href="<?= htmlspecialchars(esh_url('Uhds', 'video', ['id' => (int) ($row->id ?? 0)]), ENT_QUOTES, 'UTF-8') ?>"
                   class="btn btn-primary btn-sm py-0 px-2" title="Görüntülü görüşme">
                    <i class="fa-solid fa-video"></i>
                </a>
                <?php if (!empty($row->visit_id)): ?>
                    <a href="<?= htmlspecialchars(esh_url('Visit', 'edit', ['id' => (int) $row->visit_id]), ENT_QUOTES, 'UTF-8') ?>"
