...
<form action="<?= htmlspecialchars(esh_url('Patient', 'fsave'), ENT_QUOTES, 'UTF-8') ?>" method="post" class="needs-validation" novalidate id="patientForm" data-esh-required-legend="off">
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header bg-primary  py-3 d-flex justify-content-between align-items-center">
            <h5 class="mb-0">
                <i class="fa-solid fa-user-plus me-2"></i>Yeni Hasta Düzenleme
            </h5>
        </div>
            ...
            <div class="row g-4">
                <div class="col-lg-6">
                    <div class="card border-0 shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            <?php $eshBekleyenMode = 'bedit'; include ROOT_PATH . '/views/site/hasta/partials/edit_sections/bekleyen_kimlik.php'; ?>
                        </div>
                    </div>
                ...
                <?php include ROOT_PATH . '/views/site/hasta/partials/edit_dosya_durumu.php'; ?>
                <?php endif; ?>
                </div>
            </div>
        </div>
...
<?php
preg_match('/<select\b[^>]*>(.*)<\/select>/is', (string) ($lists['ilce'] ?? ''), $__beditMainIlce);
$beditMainIlceOptionsInner = $__beditMainIlce[1] ?? '';
?>
<div id="patient-bedit-config"
     data-main-ilce-options="<?= htmlspecialchars($beditMainIlceOptionsInner, ENT_QUOTES, 'UTF-8') ?>"
     hidden></div>
</div>
