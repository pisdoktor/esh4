    ...
    <form action="<?= htmlspecialchars(esh_url('Patient', 'store'), ENT_QUOTES, 'UTF-8') ?>" method="POST" id="patientForm" class="needs-validation" novalidate data-esh-submit-guard="off" data-esh-required-legend="off">
        ...
        <div class="row">
            <div class="col-lg-6">
                ...
                <?php if (\App\Helpers\AuthHelper::sessionIsAdmin() && \App\Helpers\EsysComplianceHelper::enabled()): ?>
                <div class="card shadow-sm border-0 mb-4 border-start border-secondary border-3">
                    <div class="card-header bg-white fw-bold py-3 text-secondary">
                        <i class="fa-solid fa-link me-2"></i> ESYS referansları
                    </div>
                    <div class="card-body">
                        <?php include ROOT_PATH . '/views/site/hasta/partials/edit_sections/esys_referans.php'; ?>
                    </div>
                </div>
                <?php endif; ?>

                <?php if (\App\Helpers\AuthHelper::sessionIsAdmin() && \App\Helpers\UsbsComplianceHelper::enabled()): ?>
                <div class="card shadow-sm border-0 mb-4 border-start border-info border-3">
                    <div class="card-header bg-white fw-bold py-3 text-info">
                        <i class="fa-solid fa-heart-pulse me-2"></i> USBS / e-Nabız referansları
                    </div>
                    <div class="card-body">
                        <?php include ROOT_PATH . '/views/site/hasta/partials/edit_sections/usbs_referans.php'; ?>
                    </div>
                </div>
                <?php endif; ?>
