...
$site_list_title = (string) ($pageTitle ?? 'Bekleyen Hastalar');
$site_list_subtitle = 'Bekleyen kayıtlar arasında arama yapın';
$site_list_icon = 'fa-solid fa-user-clock text-info';
$site_list_body_class = 'p-0';

ob_start();
?>
<form action="<?= htmlspecialchars(esh_form_action('Patient', 'listwaiting'), ENT_QUOTES, 'UTF-8') ?>" method="GET" class="esh-page__search-bar w-100">
    <?= esh_form_route_hiddens('Patient', 'listwaiting') ?>
...
?>
    ...
    <?php if (!empty($search)): ?>
        <div class="alert alert-info py-2 d-flex justify-content-between align-items-center esh-patient-list-alert mx-3 mt-3 mb-0">
            ...
            <a href="<?= htmlspecialchars(esh_url('Patient', 'listwaiting'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-info rounded-pill">Aramayı Temizle</a>
        </div>
    <?php endif; ?>
    ...
    <div class="esh-page__table-wrap esh-patient-list-table">
        <table class="table table-hover align-middle mb-0 esh-ui-table">
            <thead class="esh-page__table-head">
                <?php include __DIR__ . '/partials/list_waiting_table_head.php'; ?>
            </thead>
            <tbody>
                <?php foreach ($patients as $patient): ?>
                    <?php include __DIR__ . '/partials/list_table_row_waiting.php'; ?>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
