...
$isAdminUnified = AuthHelper::sessionIsAdmin();
if (!$isAdminUnified) {
    $statusLabels = [
        'active' => 'Aktif',
        'passive' => 'Pasif (dosya kapalı)',
...
?>
<div class="esh-page esh-page--list esh-page-patient-unified container-fluid py-4 esh-page-patient-unified<?= $curStatus === 'waiting' ? ' esh-unified-status-waiting' : '' ?>">
    <div class="row align-items-start mb-3">
        <div class="col-lg-3 mb-2">
            <h4 class="fw-bold text-dark mb-0">
                <i class="fa-solid fa-users text-primary me-2"></i><?= htmlspecialchars($pageTitle ?? 'Hasta Listesi') ?>
            </h4>
            <small class="text-muted">Durum seçin; pasif için neden ve tarih filtreleri kullanılabilir.</small>
        </div>
        <div class="col-lg-9">
            <form action="<?= htmlspecialchars(esh_form_action('Patient', 'unified'), ENT_QUOTES, 'UTF-8') ?>" method="get" class="row g-2 g-xl-3 align-items-end p-3 bg-white border rounded-3 shadow-sm esh-unified-filter">
                <?= esh_form_route_hiddens('Patient', 'unified') ?>
                <?php
                $eshUnifiedStatusOptions = [];
                foreach ($statusLabels as $k => $label) {
                    $eshUnifiedStatusOptions[] = FormHelper::makeOption((string) $k, $label);
                }
                echo FormHelper::fieldSelect('status', 'Kayıt durumu', $eshUnifiedStatusOptions, $curStatus, [
                    'col' => 'col-12 col-md-6 col-xl-' . ($isAdminUnified ? '2' : '3'),
                ...
                <?php include __DIR__ . '/partials/unified_filter_feature.php'; ?>

                <?= FormHelper::fieldInput('search', 'Arama', (string) ($search ?? ''), [
                    'col' => 'col-12 col-md-6 col-xl-' . ($isAdminUnified ? '3' : '4'),
                    'labelClass' => 'form-label small text-muted mb-1',
                ...
                <?php
                $eshUnifiedReasonOptions = [FormHelper::makeOption('', 'Tümü')];
                foreach ($pasifListesi as $k => $neden) {
                    $eshUnifiedReasonOptions[] = FormHelper::makeOption((string) $k, (string) $neden);
                }
                echo FormHelper::fieldSelect('reason', 'Pasif nedeni', $eshUnifiedReasonOptions, (string) ($reason ?? ''), [
                    'col' => 'col-12 col-md-4 col-xl-2',
                    'labelClass' => 'form-label small text-muted mb-1',
                    'class' => 'form-select-sm shadow-sm esh-filter-control',
                    'tomSelect' => false,
                ]);
                echo FormHelper::fieldDateRangeInline('startDate', 'endDate', $startDate ?? '', $endDate ?? '', [
                    'col' => 'col-12 col-md-8 col-xl-3',
                    'label' => 'Pasif tarihi',
                ]);
                ?>
                <?php endif; ?>
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <?php include __DIR__ . '/partials/unified_pdf_button.php'; ?>
...
<?php include __DIR__ . '/partials/unified_pdf_assets.php'; ?>
