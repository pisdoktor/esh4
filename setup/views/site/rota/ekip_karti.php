<div class="esh-page esh-page--list esh-page-rota container-fluid py-4">
<?php
                    ...
                    <tr <?php if($h->oncelik == 2) echo 'class="table-danger" style="--bs-table-bg: #fff5f5;"'; ?>>
                        ...
                        <td class="px-3" style="min-width: 150px;">
                            <div class="fw-bold text-dark text-nowrap">
                                <?php echo htmlspecialchars(trim((string) $h->isim . ' ' . (string) $h->soyisim), ENT_QUOTES, 'UTF-8'); ?><?php echo \App\Helpers\UIHelper::planOncelikRouteIcon($h->oncelik ?? 1); ?>
                                <?php if (!empty($h->rota_neden)): ?>
                                    <span class="ms-1 text-muted" title="<?= htmlspecialchars((string) $h->rota_neden, ENT_QUOTES, 'UTF-8') ?>" data-bs-toggle="tooltip" data-bs-placement="top">?</span>
                                <?php endif; ?>
                            </div>
