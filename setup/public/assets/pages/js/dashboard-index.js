...
function dashFieldVisitOfflineEnabled() {
    return !!(window.ESH_PAGE && window.ESH_PAGE.fieldVisit && window.ESH_PAGE.fieldVisit.offlinePlanCache);
}

function dashCacheDailyPlan(date, data) {
    if (!dashFieldVisitOfflineEnabled() || !data) {
        return;
    }
    try {
        localStorage.setItem('esh_daily_plan_' + date, JSON.stringify({ ts: Date.now(), data: data }));
    } catch (e) {
        /* depolama dolu */
    }
}

function dashLoadCachedDailyPlan(date) {
    if (!dashFieldVisitOfflineEnabled()) {
        return null;
    }
    try {
        var raw = localStorage.getItem('esh_daily_plan_' + date);
        if (!raw) {
            return null;
        }
...
function renderDailyPlanData(date, data, options) {
    options = options || {};
    if (data && !options.offline) {
        dashCacheDailyPlan(date, data);
    }
    ...
    if (options.offline) {
        contentHtml = '<div class="alert alert-warning small py-2 px-3 mb-2 border-0 shadow-sm esh-offline-banner" role="status">' +
            '<i class="fa-solid fa-wifi me-2" aria-hidden="true"></i>Çevrimdışı — son kaydedilen plan gösteriliyor.</div>' + contentHtml;
    }
...
function getDailyTasks(date) {
    ...
    $.getJSON(eshUrl('Dashboard', 'getDailyEvents', { date: date }), function (data) {
        renderDailyPlanData(date, data, { offline: false });
    }).fail(function (jqXHR, textStatus, err) {
        var cached = dashLoadCachedDailyPlan(date);
        if (cached) {
            renderDailyPlanData(date, cached, { offline: true });
            return;
        }
        dashPlanSetHtml(
