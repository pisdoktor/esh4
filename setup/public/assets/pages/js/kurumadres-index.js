(function () {
    ...
    function tipBadge(tip) {
        var map = { bolge: 'Bölge', ilce: 'İlçe', mahalle: 'Mahalle', sokak: 'Sokak' };
        return map[tip] || tip;
    }
    ...
    function resetBelow(tip) {
        var order = ['bolge', 'ilce', 'mahalle', 'sokak'];
    ...
    $(document).on('click', '.esh-kurum-adres-pick', function () {
        ...
        if (tip === 'bolge') {
            resetBelow('bolge');
            $('.esh-kurum-adres-search[data-tip="ilce"]').prop('disabled', false);
            yukle('ilce', id);
        } else if (tip === 'ilce') {
            resetBelow('ilce');
