 ...
 * Yol: templates/default/admin/stats/partials/sonda_changes_table_rows.php
 *
 * Controller : StatsController
 * Action     : sondaChangesRows (JSON)
 * Rota       : Stats/sondaChangesRows?(JSON)
 * Canonical  : views/admin/stats/partials/sonda_changes_table_rows.php (bu dosya genelde include eder)
 *
 ...
 */
/**
 * Sonda değişim takibi tablo satırları (tbody içi; yalnızca <tr>…</tr>).
...
<?php } else {
    foreach ($rows as $r): ?>
        <tr>
            ...
            <td class="x-small pe-3">
                <span class="fw-semibold <?= empty($r->sonda_degisim_tarihi) ? 'text-danger' : 'text-warning' ?>">
                    <?= !empty($r->sonda_degisim_tarihi) ? \App\Helpers\DateHelper::toTr((string) $r->sonda_degisim_tarihi) : '—' ?>
                </span>
            </td>
        </tr>
    <?php endforeach;
