 ...
 * Yol: templates/default/admin/stats/partials/supply_reports_table_rows.php
 *
 * Controller : StatsController
 * Action     : supplyReportsRows (JSON)
 * Rota       : Stats/supplyReportsRows?(JSON)
 * Canonical  : views/admin/stats/partials/supply_reports_table_rows.php (bu dosya genelde include eder)
 *
 ...
 */
/**
 * Mama / bez rapor listesi tablo satırları (tbody içi; yalnızca <tr>…</tr>).
 * @var list<object> $rows
 * @var string $tab mama|bez
 */
$bitisField = ($tab ?? 'mama') === 'bez' ? 'bezraporbitis' : 'mamaraporbitis';
if ($rows === []) { ?>
    <tr>
...
<?php } else {
    foreach ($rows as $r): ?>
        <tr>
            ...
            <td class="x-small">
                <span class="fw-semibold <?= empty($r->$bitisField) ? 'text-danger' : 'text-success' ?>">
                    <?= !empty($r->$bitisField) ? \App\Helpers\DateHelper::toTr((string) $r->$bitisField) : '—' ?>
                </span>
            </td>
