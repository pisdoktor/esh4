...
/**
 * Admin liste sayfaları — ortak açılış (WinUI / Fluent).
...
$containerClass = trim('fluent-page fluent-admin-list container-fluid admin-list-page ' . $contExtra);
?>
<div<?= $idAttr ?> class="<?= htmlspecialchars($containerClass, ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="card fluent-layer-card shadow-sm border-0<?= $cardExtra !== '' ? ' ' . htmlspecialchars($cardExtra, ENT_QUOTES, 'UTF-8') : '' ?>">
        <div class="card-header bg-white py-3 border-bottom">
            <div class="row align-items-start g-2">
                <div class="<?= $aside !== '' ? 'col-12 col-lg-4' : 'col' ?>">
                    <h5 class="fluent-page-title mb-0 text-primary">
                        <i class="<?= htmlspecialchars($icon, ENT_QUOTES, 'UTF-8') ?> me-2"></i><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?>
                    </h5>
                    <?php if ($subtitle !== ''): ?>
                        <p class="fluent-subtitle small text-muted mb-0 mt-1"><?= htmlspecialchars($subtitle, ENT_QUOTES, 'UTF-8') ?></p>
                    <?php endif; ?>
                </div>
                <?php if ($aside !== ''): ?>
