...
/**
 * Yönetim: TomTom hasta haritası (eski kök `harita.php`).
 */
class HaritaController {
    ...
    public function index(): void {
        ...
        $routeUrl = esh_url('Harita', 'tomtomRouteAjax');
        ...
        if (is_file($mahalleGeoPath)) {
            // GeoJSON public/geo altında; ASSETS_URL public/assets’tir — yanlış yol 404 verir.
    ...
    /**
     * Harita popup: ilçe · mahalle · sokak · kapı (dolu parçalar).
     */
    ...
    /** Süper yönetici — TomTom Maps SDK anahtarı (yalnızca harita ekranı). */
    ...
    /** Süper yönetici — tek hedef rota hesabı proxy. POST JSON: { locations: [[lon,lat],...] } */
