...
class UserController {
    ...
    private function listFilterState(): array {
        ...
        $eshUserListRole = $isadminFilter === 2 ? 'superadmin' : ($isadminFilter === 1 ? 'admin' : ($isadminFilter === 0 ? 'staff' : ''));
    ...
    public function changeKurum(): void
    {
        ...
        if ((int) ($userModel->isadmin ?? 0) === AuthHelper::ROLE_SUPERADMIN) {
            $_SESSION['error'] = 'Süper yönetici hesapları kuruma nakil edilemez.';
    ...
    public function store() {
        ...
        if ($targetId > 0) {
            ...
            $previousLevel = max(0, min(2, (int) $model->isadmin));
        }
        ...
        if ($requestedLevel === AuthHelper::ROLE_SUPERADMIN && $newLevel !== AuthHelper::ROLE_SUPERADMIN) {
            $_SESSION['error'] = 'Süper yönetici hesabı oluşturmak veya atamak için süper yönetici olarak oturum açmalısınız.';
        ...
        if ($newLevel === AuthHelper::ROLE_SUPERADMIN) {
            ...
            if ($reqKid <= 0) {
                $_SESSION['error'] = 'Personel ve yönetici için bir kurum seçmelisiniz.';
                header('Location: ' . ($targetId > 0 ? esh_url('User', 'adminEdit', ['id' => $targetId]) : esh_url('User', 'create')));
                exit;
            }
        ...
        if ($model->store()) {
            ...
            if ($savedId > 0 && $savedId === (int) ($_SESSION['user_id'] ?? 0)) {
                AuthHelper::syncSessionFromLevel(max(0, min(2, (int) $model->isadmin)));
                PermissionService::syncSessionPermissions($savedId, max(0, min(2, (int) $model->isadmin)));
    ...
    public function delete() {
        ...
        $level = max(0, min(2, (int) $model->isadmin));
