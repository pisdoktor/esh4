...
class AuthController {
    ...
    public function doLogin() {
        ...
        if ($userModel->loadByUsername($username)) {
            ...
            if ($userModel->verifyPasswordAndUpgrade($password)) {
                ...
                $adminLevel = max(0, min(2, (int) $userModel->isadmin));
                ...
                AuthHelper::syncSessionFromLevel(max(0, min(2, (int) $userModel->isadmin)));
    ...
    public function eimzaLogin() {
        ...
        $_SESSION['username'] = $userModel->username;
        AuthHelper::syncSessionFromLevel(max(0, min(2, (int) $userModel->isadmin)));
        $eimzaAdminLevel = max(0, min(2, (int) $userModel->isadmin));
