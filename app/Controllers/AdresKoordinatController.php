...
/**
 * Yönetim: coords boş kapı kayıtları için TomTom geocode (günlük kota ile).
 */
class AdresKoordinatController {
    ...
    public function index(): void {
        $address = new Address();
        $quota = TomtomGeocodeQuotaHelper::getSummary();
        ...
        $tomtomConfigured = defined('TOMTOM_KEY') && trim((string) TOMTOM_KEY) !== '';
    ...
    /**
     * Tek kapı: TomTom geocode (AJAX). Günlük en fazla 2500 API çağrısı.
     */
    public function processNext(): void {
        ...
        if (!defined('TOMTOM_KEY') || trim((string) TOMTOM_KEY) === '') {
            echo json_encode([
                ...
                'done' => true,
                'stop_reason' => 'no_key',
                'mesaj' => 'TomTom API anahtarı tanımlı değil.',
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }

        if (!TomtomGeocodeQuotaHelper::canMakeRequest()) {
            $quota = TomtomGeocodeQuotaHelper::getSummary();
            $address = new Address();
            echo json_encode([
                'ok' => false,
                ...
                'mesaj' => 'Bugünkü TomTom geocode kotası (' . TomtomGeocodeQuotaHelper::DAILY_LIMIT . ') doldu.',
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        $quota = TomtomGeocodeQuotaHelper::getSummary();
        $missing = $address->countKapinoMissingCoords();
        $quotaExhausted = !TomtomGeocodeQuotaHelper::canMakeRequest();
        $done = $quotaExhausted;
