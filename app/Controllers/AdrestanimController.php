...
/**
 * Admin: #__adrestablosu — hiyerarşik ilçe → mahalle → sokak → kapı (site/modules/adrestablosu ile aynı mantık).
 */
class AdrestanimController {

    private static $validTips = ['ilce', 'mahalle', 'sokak', 'kapino'];
    ...
    private function validTip($tip) {
        return in_array($tip, self::$validTips, true) ? $tip : 'ilce';
    }
    ...
    public function ajaxList() {
        $tip = $this->validTip(isset($_GET['tip']) ? (string) $_GET['tip'] : 'ilce');
    ...
    public function ajaxSave() {
        ...
        if ($tip !== 'ilce' && !$model->adminValidateParentForChild($tip, $ustId)) {
