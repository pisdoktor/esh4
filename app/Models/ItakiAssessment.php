...
/**
 * Hasta İTAKİ II düşme riski değerlendirme kayıtları.
 */
class ItakiAssessment extends BaseModel
{
    public $id = null;
    ...
    {
        parent::__construct('#__hasta_itaki', 'id');
    }

    public function ensureTable(): void
    {
        $sql = "CREATE TABLE IF NOT EXISTS #__hasta_itaki (
    ...
    public function countByHastaId(int $hastaId): int
    {
        return (int) $this->db->loadResultPrepared(
            'SELECT COUNT(*) FROM #__hasta_itaki WHERE hasta_id = :hasta',
            [':hasta' => $hastaId]
        );
    }
    ...
    {
        return $this->db->fetchObjectPrepared(
            'SELECT * FROM #__hasta_itaki WHERE id = :id',
            [':id' => $id]
        );
    }
}
