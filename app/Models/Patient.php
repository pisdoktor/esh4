...
class Patient extends BaseModel {
    ...
    public $erapor = null;
    public $esys_hasta_ref = null;
    public $esys_basvuru_ref = null;
    /** @var string|null e-Nabız hasta kayıt no */
    public $enabiz_hasta_ref = null;
    /** @var string|null USBS hasta / dosya no */
    public $usbs_hasta_ref = null;
   ...
   */
    private function waitingTenantScopeSql(?array &$params): string
    {
        ...
        if (count($ids) === 1) {
            return $this->waitingTenantScopeSqlForKurum((int) $ids[0], $params);
        }
        ...
        if (!$this->nakilTableReady()) {
            if ($params !== null) {
                ...
                return ' AND h.kurum_id IN (' . $placeholders . ')';
            }

            return ' AND h.kurum_id IN (' . $inList . ')';
        }
        ...
        if ($params !== null) {
            ...
            $hPlaceholders = implode(',', array_fill(0, count($ids), '?'));
            foreach ($ids as $id) {
                $params[] = $id;
            }
            $nPlaceholders = implode(',', array_fill(0, count($ids), '?'));

            return " AND ((h.kurum_id IN ({$hPlaceholders}) AND h.pasif = '-3') OR EXISTS (
                SELECT 1 FROM #__hasta_nakil n
                WHERE n.kaynak_hasta_id = h.id
                  AND n.durum = 'beklemede'
                  AND n.hedef_kurum_id IN ({$nPlaceholders})
                  AND n.tip IN ('kurum_ici', 'geri_nakil')
            ))";
        }

        return " AND ((h.kurum_id IN ({$inList}) AND h.pasif = '-3') OR EXISTS (
            SELECT 1 FROM #__hasta_nakil n
            WHERE n.kaynak_hasta_id = h.id
              AND n.durum = 'beklemede'
              AND n.hedef_kurum_id IN ({$inList})
              AND n.tip IN ('kurum_ici', 'geri_nakil')
        ))";
    }
     ...
     */
    private function waitingTenantScopeSqlForKurum(int $kid, ?array &$params): string
    {
        if ($kid <= 0) {
            return ' AND 1=0';
        }
        ...
        return " AND ((h.kurum_id = {$kid} AND h.pasif = '-3') OR EXISTS (
            SELECT 1 FROM #__hasta_nakil n
            WHERE n.kaynak_hasta_id = h.id
              AND n.durum = 'beklemede'
              AND n.hedef_kurum_id = {$kid}
              AND n.tip IN ('kurum_ici', 'geri_nakil')
        ))";
    }
    ...
    {
        $allowed = array_merge(
            ['', 'gecici', 'notes', 'erapor', 'esys_ref_missing'],
    ...
    private function unifiedFeatureFilterCondition(string $featureKey): string
    {
        switch ($featureKey) {
            ...
            case 'esys_ref_missing':
                if (!\App\Helpers\EsysComplianceHelper::columnsReady()) {
                    return '';
                }
                return " AND (TRIM(COALESCE(h.esys_hasta_ref, '')) = '' OR TRIM(COALESCE(h.esys_basvuru_ref, '')) = '')";
            default:
    ...
    */
    ...
    public function getPansumanlar($today_day, ?string $excludeIfIzlemOnDate = null) {
        ...
        $q = "SELECT h.id, h.isim, h.soyisim, h.tckimlik, h.izolasyon, {$eff} AS coords, h.mahalle as mahalle_id, 
