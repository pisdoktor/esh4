...
{
    /** @var list<string> */
    public const KURUM_OPERATIONAL_SECTIONS = ['map', 'planning', 'durations', 'nobet', 'vardiya'];

    /** @var list<string> */
    private const CORPORATE_FLAT_KEYS = ['esh_app_name', 'ek3_form_baslik', 'hekim_degerlendirme_form_baslik'];
     ...
     *
     * @return array{mode:string,label:string,hint:string,kurum_id?:int}|null
     */
    public static function adminScopeBanner(): ?array
    {
        ...
        $kurumId = self::writeKurumId();
        if ($kurumId !== null && $kurumId > 0) {
            $name = self::kurumName($kurumId);

            return [
                'mode' => 'kurum',
                'kurum_id' => $kurumId,
                'label' => $name !== '' ? $name : ('Kurum #' . $kurumId),
                'hint' => 'Kayıtlar yalnızca bu kuruma yazılır. Tanımlı olmayan alanlar platform varsayılanını kullanır.',
            ];
        }
        if (AuthHelper::sessionIsSuperAdmin()) {
            return [
                ...
                'hint' => 'Kurum filtresi seçili değil — değişiklikler tüm kurumlar için varsayılan değer olur. Belirli bir kurumu düzenlemek için üst menüden kurum seçin.',
            ];
        }
