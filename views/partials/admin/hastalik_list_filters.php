...
$searchQ = isset($searchQ) ? trim((string) $searchQ) : '';
        ...
        <div class="card-header bg-white py-3 px-3 px-md-4 border-bottom d-flex flex-wrap align-items-center justify-content-between gap-3">
            <div class="d-flex flex-wrap align-items-center gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <span class="small text-muted"><?= $isCatalogPickerMode ? 'Kategori süzgeci ağaç yüklemesine uygulanır.' : 'Kategori ve arama ile «Filtrele» veya ağaç içi arama.' ?></span>
                </div>
            </div>
            <button
        ...
        <div id="hastalik-filter-collapse" class="collapse<?= $filterExpanded ? ' show' : '' ?>">
            <div class="card-body p-3 p-md-4 bg-body-tertiary bg-opacity-25">
                ...
                <?= esh_form_route_hiddens('Hastalik', 'index') ?>
                    ...
                    <?php
                    $eshHastalikCatOptions = [
                        FormHelper::makeOption('', 'Tüm kategoriler'),
                        FormHelper::makeOption('0', 'Kategorisiz'),
                    ];
                    foreach ($categories as $cat) {
                        $cid = (string) (int) ($cat->id ?? 0);
                        if ($cid === '0') {
                            continue;
                        }
                        $eshHastalikCatOptions[] = FormHelper::makeOption($cid, (string) ($cat->name ?? ''));
                    }
                    echo FormHelper::fieldSelect('cat', 'Kategori', $eshHastalikCatOptions, $catVal, [
                        'col' => 'col-12 col-sm-6 col-lg-4 col-xl-3',
                        'id' => 'hastalik-filter-cat',
                    ...
                    if (!$isCatalogPickerMode) {
                        echo FormHelper::fieldInput('q', 'ICD / tanı ara', $searchQ, [
                            'col' => 'col-12 col-sm-6 col-lg-4 col-xl-3',
                            'id' => 'hastalik-filter-q',
                            'labelClass' => 'form-label fw-semibold small text-secondary mb-1',
