...
<div class="card esh-hasta-kimlik-kart border-0 shadow-sm mb-3" style="background: <?= htmlspecialchars($kimlikCardBg, ENT_QUOTES, 'UTF-8') ?>;">
    <div class="card-body pt-3 pb-4">
        ...
        <div class="esh-hasta-kimlik-kart__continuation vstack gap-3 border-top border-dark border-opacity-10 pt-4 mt-4">
            ...
            <?php
            $barthelTotal = 0;
            foreach (['barbeslenme', 'barbanyo', 'barbakim', 'bargiyinme', 'barbarsak', 'barmesane', 'bartuvalet', 'bartransfer', 'barmobilite', 'barmerdiven'] as $barthelField) {
                $barthelTotal += (int) ($hasta->$barthelField ?? 0);
            }
            $barthelStatus = 'Bağımsız';
            if ($barthelTotal <= 20) {
                $barthelStatus = 'Tam Bağımlı';
            } elseif ($barthelTotal <= 60) {
                $barthelStatus = 'Ağır Bağımlı';
            } elseif ($barthelTotal <= 90) {
                $barthelStatus = 'Orta Bağımlı';
            }
