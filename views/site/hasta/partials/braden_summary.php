<?php
/**
 * Hasta kartı — Braden ölçeği özet kartı.
 *
 * @var object $hasta
...
$riskMeta = $latestTotal > 0 ? BradenScaleHelper::resolveRisk($latestTotal) : ['label' => '—', 'badgeClass' => 'bg-secondary-subtle text-secondary border'];
$tarihTr = $latest && !empty($latest->degerlendirme_tarihi)
    ? \App\Helpers\DateHelper::toTrOrEmpty((string) $latest->degerlendirme_tarihi)
    : '';
?>
<div class="card esh-braden-summary-card shadow-sm border-0 border-top border-danger border-3 rounded mb-0">
    <div class="card-body p-3">
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
            ...
            <a href="<?= htmlspecialchars($bradenUrl, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-danger btn-sm fw-semibold shadow-sm">
        ...
        <?php else: ?>
            <p class="small text-muted mb-2">
                ...
                Bası yarası risk takibi için Braden değerlendirmesi ekleyebilirsiniz.
            </p>
        <?php endif; ?>

        <?php if (empty($pasifDosyaKapali ?? false)): ?>
            <a href="<?= htmlspecialchars($bradenUrl, ENT_QUOTES, 'UTF-8') ?>#patientBradenForm" class="btn btn-outline-danger btn-sm">
