            ...
            <?php
            $catalogPickerShuttleId = 'esh-istek-shuttle';
            $catalogPickerShuttleFetchUrl = $indexRowsFetchUrl ?? '';
            $catalogPickerShuttleCatalogLabel = 'Platform başvuru amaçları (esh_istekler)';
            $catalogPickerShuttleAssignedLabel = 'Kurum başvuru amaçları';
            ...
            <?php else: ?>
            <p class="small text-muted mb-3">Platform geneli katalog. Kurumlar bu listeden kendi seçeneklerini belirler.</p>
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    <thead class="table-light">
                        <tr>
                            <?= \App\Helpers\UIHelper::renderSortTh('ID', 'id', $ordering, $eshSortCfg) ?>
                            <?= \App\Helpers\UIHelper::renderSortTh('Başvuru amacı (metin)', 'name', $ordering, $eshSortCfg) ?>
                            <th width="150" class="text-center">İşlemler</th>
                        </tr>
                    </thead>
                    <tbody id="esh-istek-list-tbody"
                           data-esh-fetch-url="<?= htmlspecialchars($indexRowsFetchUrl ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           data-esh-picker-mode="0">
                        <tr class="esh-istek-list-loading-row">
