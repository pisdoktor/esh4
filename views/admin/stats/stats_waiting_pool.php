<div class="esh-page esh-page--list esh-page-stats container-fluid py-4">
<?php include __DIR__ . '/partials/stats_waiting_pool/bootstrap.php'; ?>
<div class="container-fluid mt-3 esh-stats-report">
    <?php require \App\Helpers\ThemeViewHelper::resolvePartial('admin/stats_breadcrumb'); ?>
<?php include __DIR__ . '/partials/stats_waiting_pool/report_header.php'; ?>
<?php include __DIR__ . '/partials/stats_waiting_pool/main_content.php'; ?>
</div>
</div>
