    <div class="card border-0 shadow-sm mb-3">
        <div class="card-header bg-white border-bottom py-3">
            <form class="row g-3 align-items-end" method="get" action="<?= htmlspecialchars(esh_form_action('Stats', 'birIzlemliler'), ENT_QUOTES, 'UTF-8') ?>">
                <?= esh_form_route_hiddens('Stats', 'birIzlemliler') ?>
                <?php
                $eshBirIzlemIlceOptions = [\App\Helpers\FormHelper::makeOption('0', 'Tüm ilçeler')];
                foreach ($ilceler as $ic) {
                    $eshBirIzlemIlceOptions[] = \App\Helpers\FormHelper::makeOption((string) ($ic->id ?? ''), (string) ($ic->adi ?? ''));
                }
                echo \App\Helpers\FormHelper::fieldSelect('ilce', 'İlçe', $eshBirIzlemIlceOptions, ($ilce === '' ? '0' : (string) $ilce), [
                    'col' => 'col-12 col-md-4',
                ...
                $eshBirIzlemMahalleOptions = [\App\Helpers\FormHelper::makeOption('0', 'Tüm mahalleler')];
                foreach ($mahalleler as $mh) {
                    $eshBirIzlemMahalleOptions[] = \App\Helpers\FormHelper::makeOption((string) ($mh->id ?? ''), (string) ($mh->adi ?? ''));
                }
                echo \App\Helpers\FormHelper::fieldSelect('mahalle', 'Mahalle', $eshBirIzlemMahalleOptions, ($mahalle === '' ? '0' : (string) $mahalle), [
                    'col' => 'col-12 col-md-4',
                    'labelClass' => 'form-label fw-semibold small text-secondary mb-1',
                    'class' => 'form-select-sm shadow-sm',
                    'tomSelect' => false,
                ...
                <div class="col-12 col-md-4 d-flex gap-2 flex-wrap">
                    ...
                    <a href="<?= htmlspecialchars(esh_url('Stats', 'birIzlemliler'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-secondary btn-sm rounded-pill px-3">Sıfırla</a>
                </div>
            </form>
        </div>
        <?php \App\Helpers\StatsViewPdfHelper::renderCardHeader('<i class="fa-solid fa-chart-gantt text-primary me-2"></i>Sadece tek izlemi olan hastalar', 'main', 'h5', 'card-header bg-light border-bottom py-3'); ?>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-sm table-hover mb-0 align-middle">
                    ...
                    <tbody id="esh-bir-izlemliler-list-tbody"
                           data-esh-fetch-url="<?= htmlspecialchars($birIzlemlilerRowsFetchUrl ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        <tr class="esh-bir-izlemliler-list-loading-row">
                            <td colspan="9" class="border-0 py-5 text-center text-muted">
                                <div class="d-flex flex-column align-items-center gap-2">
                                    <span class="spinner-border spinner-border-sm text-primary" role="status" aria-hidden="true"></span>
                                    <span>Bir izlem listesi yükleniyor…</span>
                                </div>
                            </td>
                        </tr>
                    </tbody>
