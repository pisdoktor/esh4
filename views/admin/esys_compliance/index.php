...
?>
<div class="esh-page esh-page--list esh-page-esys container-fluid py-4">
    <?php if (!$columnsReady): ?>
        <div class="alert alert-warning" role="alert">
            ESYS referans sütunları henüz kurulmamış. Yönetici olarak
            <code>database/migrate_esh_esys_refs.sql</code> dosyasını çalıştırın.
        </div>
    <?php endif; ?>
    ...
    <header class="esh-page__header mb-4">
        <h1 class="esh-page__heading h4 mb-1">
            ...
            <?= htmlspecialchars(EsysComplianceHelper::mappingTitle(), ENT_QUOTES, 'UTF-8') ?>
        </h1>
        <?php if (EsysComplianceHelper::mappingDescription() !== ''): ?>
            <p class="esh-page__lead small text-muted mb-0">
                <?= htmlspecialchars(EsysComplianceHelper::mappingDescription(), ENT_QUOTES, 'UTF-8') ?>
            </p>
        <?php endif; ?>
    </header>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <div class="text-muted small">ESYS ref eksik hasta</div>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <div class="text-muted small">Yapıldı izlem — ref eksik</div>
                    <div class="fs-4 fw-bold text-warning"><?= (int) ($kpis['visits_missing'] ?? 0) ?></div>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <div class="text-muted small">Bekleyen plan — ref eksik</div>
                    <div class="fs-4 fw-bold text-secondary"><?= (int) ($kpis['plans_missing'] ?? 0) ?></div>
    ...
    <div class="alert alert-info border-0 shadow-sm mb-4" role="note">
        <strong>Manuel köprü:</strong> Hasta düzenleme ve izlem formlarındaki
        <em>ESYS referans no</em> alanları, resmi entegrasyon gelene kadar çift kaydı azaltmak için kullanılır.
        Eşleme tanımı <code>config/esys-field-mapping.json</code> dosyasından okunur.
        <?php if (\App\Helpers\EsysBridgeHelper::isReady()): ?>
            <div class="mt-2">
                <a href="<?= htmlspecialchars(esh_url('EsysBridge', 'index'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-primary rounded-pill">
                    <i class="fa-solid fa-bridge me-1"></i>ESYS / AHBS dosya köprüsü
                </a>
            </div>
        <?php endif; ?>
    </div>
    ...
    <?php foreach ($sections as $section): ?>
        ...
        $refCols = is_array($data['esys_ref_columns'] ?? null) ? $data['esys_ref_columns'] : [];
                        ...
                        <tbody>
                            <?php foreach ($fields as $field): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="font-monospace small"><?= htmlspecialchars((string) ($field['esys'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>
                                    <td class="small">
                                    ...
                                    <td class="small text-muted">
                                        ...
                                        if (!empty($field['entity'])) {
                                            $notes[] = 'Kaynak: ' . (string) $field['entity'];
                                        }
