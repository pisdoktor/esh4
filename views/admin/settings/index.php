 ...
 * @var array{mode:string,label:string,hint:string,kurum_id?:int}|null $settingsScopeBanner
 */
...
$showSaveButton = $activeTab !== 'eimza';
...
<div class="esh-page esh-page--list esh-page-settings <?= htmlspecialchars($settingsPageModifier, ENT_QUOTES, 'UTF-8') ?> container-fluid py-4">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-end gap-2 mb-4">
        <div>
            ...
            <p class="text-muted mb-0 small"><?= \App\Helpers\AuthHelper::sessionIsSuperAdmin() ? 'Modül, harita, planlama, kurumsal ad ve güvenlik seçenekleri.' : 'Kurumunuza özel modül, işlem id, harita, planlama, kurumsal metin ve nöbet ayarları.' ?></p>
        </div>
    </div>
    ...
    <?php if (is_array($settingsScopeBanner) && ($settingsScopeBanner['label'] ?? '') !== '' && in_array($activeTab, ['modules', 'islem_ids', 'harita', 'planlama', 'kurumsal', 'nobet'], true)): ?>
        ...
        $scopeAlert = $scopeMode === 'kurum' ? 'alert-info' : 'alert-secondary';
        ?>
        <div class="alert <?= htmlspecialchars($scopeAlert, ENT_QUOTES, 'UTF-8') ?> small mb-3">
            <strong><?= $scopeMode === 'kurum' ? 'Kurum kapsamı:' : 'Kayıt hedefi:' ?></strong>
