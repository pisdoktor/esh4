        <div class="col-xl-4">
            <div class="card shadow-sm border-0 h-100">
                <div class="card-header bg-white fw-bold">Muafiyet İsteği Ekle</div>
                <div class="card-body">
                    <form method="post" action="<?= htmlspecialchars(esh_url('Nobet', 'saveIstek'), ENT_QUOTES, 'UTF-8') ?>" class="row g-2">
                        <div class="col-12">
                            <select class="form-select form-select-sm" name="personel_id" required>
                                <option value="">Personel seçiniz</option>
                                <?php foreach ($personeller as $p): ?>
                                    <option value="<?= (int) $p->id ?>"><?= htmlspecialchars((string) $p->name) ?> (<?= htmlspecialchars((string) $p->unvan) ?>)</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-6"><input type="text" name="baslangic_tarihi" class="form-control form-control-sm datepicker" placeholder="Başlangıç" required></div>
                        <div class="col-6"><input type="text" name="bitis_tarihi" class="form-control form-control-sm datepicker" placeholder="Bitiş" required></div>
                        <div class="col-12"><input type="text" name="aciklama" class="form-control form-control-sm" placeholder="Açıklama"></div>
                        <div class="col-12"><button class="btn btn-sm btn-info text-white">Kaydet</button></div>
                    </form>
                </div>
            </div>
        </div>
