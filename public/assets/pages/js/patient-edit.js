...
$(document).ready(function () {
    function calculateBarthel() {
        let total = 0; $('.barthel-input').each(function () { total += parseInt($(this).val()) || 0; });
        $('#barthel-total-badge').text('Toplam Skor: ' + total);
        let status = '', colorClass = ''; if (total <= 20) { status = 'Tam Bağımlı'; colorClass = 'text-danger'; } else if (total <= 60) { status = 'İleri Derecede Bağımlı'; colorClass = 'text-warning'; } else if (total <= 90) { status = 'Orta Derecede Bağımlı'; colorClass = 'text-info'; } else if (total <= 99) { status = 'Hafif Derecede Bağımlı'; colorClass = 'text-primary'; } else { status = 'Tam Bağımsız'; colorClass = 'text-success'; }
        $('#bagimlilik-input').val(total + ' Puan - ' + status);
    }
    $(document).on('input change', '.barthel-input', calculateBarthel);
    calculateBarthel();
