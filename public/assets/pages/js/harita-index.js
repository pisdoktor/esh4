(function () {
    ...
    var mapKeyUrl = String(cfg.mapKeyUrl || '');
    if (!mapKeyUrl) {
        if (window.toastr) {
    ...
    fetch(mapKeyUrl, { credentials: 'same-origin' })
        .then(function (r) { return r.json(); })
        .then(function (keyPayload) {
            if (!keyPayload || !keyPayload.ok || !keyPayload.key) {
                if (window.toastr) {
                    toastr.error('TomTom API anahtarı tanımlı değil (TOMTOM_KEY).');
                }
                return;
            }
            bootHarita(String(keyPayload.key));
        })
        .catch(function () {
            if (window.toastr) {
        ...
        features.forEach(function (feature) {
            if (feature.properties && !feature.properties.cluster) {
                ...
                if (!markersOnTheMap[fid]) {
                    ...
                    var m = new tt.Marker({ color: color }).setLngLat(feature.geometry.coordinates);
                    m.setPopup(new tt.Popup({ offset: 30, maxWidth: '320px' }).setHTML(feature.properties.html));
                    m.addTo(map);
    ...
    function popupAc(lng, lat, html) {
        popupKapat();
        aktifPopup = new tt.Popup({ offset: 18, maxWidth: '320px' })
            .setLngLat([lng, lat])
            .setHTML(html)
            .addTo(map);
    }
    ...
    function rotaHesapla(hedef) {
        ...
        fetch(routeUrl, {
            ...
            .then(function (r) { return r.json(); })
            .then(function (data) {
                if (!data || !data.ok || !data.route || !data.route.legs || !data.route.legs[0]) {
                    throw new Error('Rota yanıtı geçersiz');
                }
                var pts = data.route.legs[0].points.map(function (p) { return [p.longitude, p.latitude]; });
                map.getSource('route-source').setData({
                    type: 'Feature',
                    geometry: { type: 'LineString', coordinates: pts }
                });

                var mesafe = (data.route.summary.lengthInMeters / 1000).toFixed(1);
