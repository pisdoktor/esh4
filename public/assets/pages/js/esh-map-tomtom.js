(function (global) {
    ...
    function createBounds(lngLatLib) {
        return new lngLatLib.LngLatBounds();
    }

    function boot(config) {
        var map = global.tt.map({
            key: config.key,
        ...
        return {
            ...
            raw: map,
            boundsClass: global.tt.LngLatBounds,
            onLoad: function (fn) {
                if (map.loaded()) {
                    fn();
                } else {
            ...
            addNavigationControl: function (position) {
                map.addControl(new global.tt.NavigationControl(), position || 'top-right');
            },
            addFullscreenControl: function () {
                map.addControl(new global.tt.FullscreenControl());
            },
            addMarker: function (lngLat, options) {
                options = options || {};
                var m = new global.tt.Marker({
                    color: options.color || '#3388ff',
                    anchor: options.anchor || 'bottom'
                }).setLngLat(lngLat);
                if (options.popupHtml) {
                    m.setPopup(new global.tt.Popup({ offset: options.popupOffset || 30, maxWidth: options.maxWidth || '320px' }).setHTML(options.popupHtml));
                }
                m.addTo(map);
                return m;
            },
            addPopup: function (lngLat, html, options) {
                options = options || {};
                return new global.tt.Popup({ offset: options.offset || 18, maxWidth: options.maxWidth || '320px' })
                    .setLngLat(lngLat)
                    .setHTML(html)
                    .addTo(map);
            },
            ...
            createBounds: function () { return createBounds(global.tt); }
        };
    }

    global.EshMapAdapters = global.EshMapAdapters || {};
    global.EshMapAdapters.tomtom = { create: boot };
})(window);
